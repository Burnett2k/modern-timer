...
function startTimerWithSettings({
  ...
  const totalSecondsRequested: number = convertTimeToSeconds(
    hours,
    minutes,
    seconds
  );
  ...
  const remainingSecondsToCount: number =
  ...
  intervalTimerId = setInterval(() => {
    const elapsedTimeInSeconds: number = Math.floor(
      (Date.now() - timerState.timerStartTimestamp) / MILLISECONDS_IN_SECOND
    );
    const remainingSecondsOnTimer: number = Math.max(
      0,
      timerState.totalDurationInSeconds - elapsedTimeInSeconds
    );
...
function pauseCurrentTimer(): void {
  ...
  timerState.pausedTimeRemainingInSeconds = Math.max(
    0,
    timerState.totalDurationInSeconds - elapsedTimeInSeconds
  );
...
function setTimerDuration({
  ...
  timerState.totalDurationInSeconds = convertTimeToSeconds(
    hours,
    minutes,
    seconds
  );
...
function convertSecondsToTimeObject(totalSecondsToConvert: number): TimeObject {
  ...
  const minutes: number = Math.floor(
    (totalSecondsToConvert % SECONDS_IN_HOUR) / SECONDS_IN_MINUTE
  );
...
function sendTimerTickMessage(remainingSecondsOnTimer: number): void {
  const { hours, minutes, seconds }: TimeObject = convertSecondsToTimeObject(
    remainingSecondsOnTimer
  );
